...
RQ6 & scalability ($|H|$ 1k..80k) & \code{scale.exs} ($\rightarrow$ \code{Harness.scale/1}) & \code{results\_scale.csv} $\rightarrow$ \code{eval\_scale.pdf} \\
...
handoff, jejak operator)?'' \code{Harness.overhead/1} mengukur ketiganya. Hasil tercatat (median 5×
saat mesin idle): probe $\sim$4 µs, $\sim$6 KB/proses, $\sim$8k handoff/detik; operator di kind
$\sim$73 MiB dan $\sim$74 ms reconcile/5 detik.
...
mana plafon per-node?'' \code{scale.exs} menyapu $|H| = 1\text{k} \rightarrow 80\text{k}$. Memori naik
linear dan murah ($\sim$4 KB/proc), tetapi throughput handoff efektif \emph{ambruk super-linear}
(3367 $\rightarrow$ 152 $\rightarrow$ 57 $\rightarrow$ 18 proc/detik) saat registry delta-CRDT terisi.
Plafon per-node $\approx$30--35k: di bawah anggaran 600 detik, node membuang 5.632/40.000 dan
69.038/80.000 proses. Figur: \code{eval\_scale.pdf}.
...
ambruk (21.700 $\rightarrow$ 6{,}7/detik), dan grace naik hingga plafon $g_{\max}$. Figur:
...
\subsection{RQ6 --- scalability ($|H|$ 1k..80k)}
\begin{lstlisting}[style=sh]
cd code/app
