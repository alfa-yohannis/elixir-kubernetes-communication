%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Paper -- Target: Software: Practice and Experience (Wiley), Q2
...
\title{Closing the Grace Gap: Loss-Free Pod Shutdown for Stateful Clustered
...
\titlemark{Closing the Grace Gap: Loss-Free Pod Shutdown on Kubernetes}
...
\abstract[Abstract]{%
Stateful services increasingly run on container orchestrators that add their own recovery on top of the
application runtime's. Such a service can be upgraded, or a machine can be withdrawn for maintenance. An
instance about to be removed must first migrate its in-memory state to surviving instances and let the
cluster settle before it disappears. To bound this shutdown, the orchestrator grants a fixed,
operator-chosen \emph{grace period} and then forcibly terminates the instance. We argue that a fixed
grace period is a poor fit for this task. Set it too low and forced termination interrupts
the migration, losing state and availability. Set it defensively high and every instance slows the
rollout. The time actually required is not a constant. It grows with the state to migrate and
the load on the cluster, so no single value is right for all instances or moments. We present a
cross-layer feedback controller that \emph{derives} the grace period at run time from the runtime's own
progress signals, and paces the rollout accordingly. We formalise a safety condition linking the
shutdown window to the runtime's migration and convergence time. We provide an open reference
implementation on Kubernetes for Erlang/BEAM clusters, and evaluate it against fixed and
over-provisioned baselines under fault injection. A fixed short grace loses about 25\% of the
in-flight state once migration outlasts it. The controller loses 0\%, while reserving up to
$\sim$$19\times$ less time than a defensive worst-case grace, keeping upgrades both safe and fast.}
...
The Erlang BEAM virtual machine (the runtime that executes Erlang and Elixir programs) is often used
through Elixir and the OTP framework (a standard library of patterns for building reliable systems).
Services built on it are organised as large numbers of lightweight processes. These processes run under
supervision trees (hierarchies that watch child processes) that restart failed processes in
microseconds. A growing number of these services are deployed on Kubernetes (a system that runs and
manages containers across many machines). Kubernetes adds a second, coarser tier of fault tolerance: it
restarts containers, reschedules pods (the smallest deployable units, each holding one or more
containers), and replaces nodes. The two tiers are complementary. They operate at different
granularities and timescales, and stateful BEAM applications routinely rely on both at once. Process
state that must survive a pod's disappearance is held in distributed registries such as
Horde~\cite{horde} (a library that keeps process identity and state across a cluster). It is made
discoverable through cluster-formation libraries such as libcluster~\cite{libcluster} (which connects
the runtime nodes into one cluster).

The two tiers must nonetheless cooperate at one specific moment: when a pod is deliberately removed.
This happens during a rolling update (replacing pods a few at a time to roll out a new version) or a
node drain (moving every pod off a machine). Kubernetes then terminates a pod in fixed steps. It sends
\texttt{SIGTERM} (a polite request to shut down), optionally runs a \texttt{preStop} hook (a command
run just before shutdown), waits for a bounded \emph{grace period}
(\texttt{terminationGracePeriodSeconds}), and then sends \texttt{SIGKILL} (a forced kill that cannot be
caught)~\cite{k8sgraceful}. A stateful BEAM pod must use that window to \emph{hand off} its processes to
surviving pods (move their in-memory state elsewhere). It must also let the cluster re-converge
(membership stabilises, registries reconcile, singletons re-register) before it disappears. The grace
period therefore acts as a deadline on cross-tier handoff.

The difficulty is that this deadline is a \emph{static constant chosen by the operator at deploy
time}. Suppose it is shorter than the handoff plus convergence time. Then \texttt{SIGKILL} interrupts the
handoff and state is lost. Registered singletons vanish and requests fail until the cluster
re-converges. Suppose instead it is set defensively high. Then the cost reappears at the other end. A
rolling update advances only as fast as each pod's grace allows, so an over-long grace makes every
deployment slow and resource-hungry. Crucially, the time actually required is not fixed. It grows with
the number of stateful processes a pod holds, shrinks with the achievable handoff rate, and depends on
cluster convergence time. All of these vary with load. A single constant cannot be both safe and
...
We close that gap with a \emph{grace-convergence controller}. It is a cross-layer feedback loop that
...
membership-convergence state. A Kubernetes operator (a controller that automates operational tasks)
uses these to compute a per-pod grace period and to pace the rolling update. An adaptive
\texttt{preStop} hook drains and then blocks precisely until handoff completes, instead of sleeping for
a guessed interval. The result is a deadline that is just long enough to be safe and no longer.

This paper makes the following contributions:
\begin{enumerate}
  \item A \emph{grace-safety invariant} that formalises, across the orchestrator/runtime boundary,
        when a graceful termination is safe (Section~\ref{sec:model}). We give a policy that provably
  ...
  \item The design of a cross-layer grace-convergence controller. It is decomposed into
        runtime-agnostic roles so that the principle transfers beyond the BEAM (Section~\ref{sec:design}).
  \item A reference implementation as an open artifact. It is built from libcluster, Horde, a Kubernetes
  ...
  \item An empirical evaluation on Kubernetes against static, over-provisioned, and fixed-sleep
        baselines (Section~\ref{sec:eval}). It measures state loss, rollout time, adaptivity, controller
        overhead, scalability to a per-node handoff ceiling, behaviour under injected real network
        latency, fail-safe response to faults, and a realistic-workload case study on Phoenix.Presence.
\end{enumerate}
...
\emph{Terminating} and removed from service endpoints. An optional \texttt{preStop} hook runs, and the
container receives \texttt{SIGTERM}. After at most \texttt{terminationGracePeriodSeconds} (default
...
bounded by \texttt{maxUnavailable} and \texttt{maxSurge}. A \emph{node drain} (for maintenance or
...
A BEAM application is a cluster of nodes (one per pod, typically). Within a node, its processes are
supervised by OTP. Across nodes, they are discovered through a membership layer such as
libcluster~\cite{libcluster}. That layer forms the cluster from Kubernetes endpoints and must
\emph{converge} after any topology change (settle into a consistent view of who is in the cluster).
Stateful work is commonly placed in a distributed registry and supervisor such as Horde~\cite{horde}.
Horde uses delta-CRDTs (a data structure that lets replicas merge changes without conflicts). So when a
node leaves, the processes it owned can be \emph{handed off} and restarted on surviving nodes, and
...
\emph{within} its grace period. Otherwise \texttt{SIGKILL} will cut it off mid-handoff
(Figure~\ref{fig:problem}). Consider an illustrative deployment. A pod hosts a few hundred
...
re-converges. Raising the grace to, say, 300\,s removes the loss. But it makes every pod in a rolling
update wait up to five minutes before the next can proceed. Because the required time tracks load,
neither constant is right in general. This motivates a grace period that is computed from the runtime's
...
trades one cost for another. A rolling update advances no faster than the grace each pod
actually consumes, so an over-long grace inflates every deployment and ties up capacity. It is also
...
\paragraph{Readiness and liveness probes.} Probes~\cite{k8sgraceful} (periodic health checks
Kubernetes runs on a pod) gate traffic and trigger restarts from a per-pod health signal. They are
separate from termination. They cannot lengthen, shorten, or otherwise coordinate the kill deadline, so
they say nothing about whether a departing pod has finished the handoff of Figure~\ref{fig:problem}.

\paragraph{PodDisruptionBudgets.} A PDB~\cite{k8sPDB} (a PodDisruptionBudget, a rule capping how many
pods can be down at once) limits how many pods can be voluntarily disrupted at once. That is an
availability \emph{count} constraint, not a per-pod completion check. It does not know whether a given
pod has drained its state, and it never extends that pod's grace. So a pod can still be
\texttt{SIGKILL}ed mid-handoff while the budget is satisfied.
...
way to influence. Sometimes handoff legitimately needs longer than the configured grace (as in
Figure~\ref{fig:problem}). Then doing everything right internally does not help: the orchestrator kills
it regardless.

\paragraph{Runtime-native coordinated shutdown.} Actor runtimes offer phased shutdown. Examples are
Akka's \emph{CoordinatedShutdown}~\cite{akkacoordinated} and Orleans' graceful
deactivation~\cite{orleans} (Section~\ref{sec:related}). These run ordered phases with \emph{configured}
per-phase timeouts. Those timeouts live inside the runtime and are never communicated to the
orchestrator's grace period. The phases are better-structured guesses, but they are still guesses, and
the cross-layer deadline stays uninformed.
...
coordinate cluster \emph{membership and quorum} (agreement on a majority of members) across the two
layers~\cite{akkaLease,akkaSBR}. Autonomic and operator-driven controllers manage \emph{placement and
lifecycle}~\cite{kephart2003, meliani2025proactive}. Rollback-recovery and checkpointing \emph{preserve
state} for restoration elsewhere~\cite{elnozahy2002}. And deployment-time optimisation plans placement
under fault-tolerance and latency constraints~\cite{ucc23,gustafsson2025rtilience}. Each is genuinely
cross-layer, yet none couples the termination grace period to the runtime's handoff and convergence
progress. So in Figure~\ref{fig:problem} the pod is still killed at 30\,s with its handoff unfinished.

\medskip\noindent
Every option above either freezes a constant ahead of time or acts within a single layer. None
establishes a feedback path across the orchestrator/runtime boundary (Table~\ref{tab:existing}). As
Section~\ref{sec:intro} set out, that boundary is where the information that should size the deadline
and the control that enforces it are split. Building that path is exactly what our controller does
...
We consider a BEAM application running as a Kubernetes \emph{Deployment} of $n$ pods. Each pod is a
...
\begin{enumerate}
  ...
  \item allow the cluster to \emph{re-converge}---membership stabilises and registries reconcile---
        taking time $T_c$.
\end{enumerate}
Throughout, \emph{drain} refers narrowly to the first step (finishing in-flight work, $T_d$) and
\emph{handoff} to the second (relocating in-memory state, $|H_p|/\rho$). The two are distinct and
additive, and ``a drain'' in the implementation denotes one full pass of the handoff loop.
...
the load term $|H_p|/\rho(p)$ is large. This produces exactly the truncated-handoff loss of
Section~\ref{sec:bg-gap}. Over-provisioning ($g_0$ set to a large worst-case, e.g.\ 300\,s) restores
safety but is bad for throughput. In a rolling update the wall-clock time is bounded below by the
...
\end{equation}
The right-hand side is known only to the runtime, and only at termination time. So achieving
$g \approx g^\star$ requires feeding runtime state across the boundary to the orchestrator. This is the
...
%====================================================================
% Reader-facing title and text carry no internal mechanism/milestone label.
...
We describe the design in terms of \emph{roles}, meaning runtime-agnostic
responsibilities. We defer the concrete components that realise each role to
...
Coordinator (operator) & Watch pods; compute and patch the grace period; pace \texttt{maxUnavailable} & \texttt{kubectl}-based operator \\
...
is smoothed with an exponentially weighted moving average (a running average that weights recent samples
more) over recent handoff activity. This way a transient stall does not produce a wildly large estimate.
The probe is read-only and side-effect free, so the coordinator can poll it without perturbing the
runtime.

\subsection{Coordinator and grace computation}\label{sec:design-coord}
The coordinator is a Kubernetes operator that watches Deployments. When a rolling update or drain
is about to remove a pod, it reads that pod's probe and computes a grace period from
...
in-flight work and then blocks while handoff is pending. It returns as soon as the backlog is cleared
(Figure~\ref{fig:act}). The coordinator has already sized the grace period to cover the
expected handoff, so this block almost always returns well before $g_{\max}$. The cap exists only as a
backstop.
...
and pacing are idempotent (repeating them has no extra effect), so a coordinator restart during a
rollout does not corrupt the update. The mechanism addresses \emph{voluntary} disruptions (rolling
updates, drains, scale-down). Sudden, non-graceful node death gives no grace window and is out of scope
here. Coordinating disruption \emph{budgets} with cluster quorum is a complementary, separate mechanism
that we do not address in this paper.

\subsection{Safety guarantee}\label{sec:design-safety}

The policy is constructed so that, under a conservative rate estimate, it satisfies the grace-safety
invariant of Equation~\eqref{eq:invariant}. The argument is deliberately elementary. It follows
directly from the clamp in Algorithm~\ref{alg:grace}, and we claim no heavier formal machinery. Its
value is not depth but precision. It pins down exactly the two assumptions under which the controller
is safe and, more usefully, the only two ways it can fail.
...
(Section~\ref{sec:design-failsafe}). Violating it with an over-optimistic rate lets the policy
...
construction. The empirical check of Section~\ref{sec:eval} (Figure~\ref{fig:invariant}) confirms
...
(Table~\ref{tab:roles}) to concrete components. Cluster \emph{membership} maps to libcluster,
cluster-wide process \emph{identity} and state \emph{handoff} to Horde, the per-pod \emph{probe} and
drain endpoint to Bandit/Plug, and the \emph{coordinator} to a Kubernetes operator. A single release
image runs in two roles, selected by the \texttt{GRACE\_ROLE} environment variable.

\subsection{Stateful workload, identity, and handoff}\label{sec:impl-handoff}
Each stateful unit is a \texttt{GenServer} (an OTP process that holds state and answers messages)
holding in-memory state. Workers are \emph{hosted} by a node-local \texttt{DynamicSupervisor} but
registered for \emph{identity} in a cluster-wide \texttt{Horde.Registry} (a $\delta$-CRDT). We
deliberately separate hosting from identity. Horde's distributed supervisor restarts a process at its
own ring placement, which can re-home it \emph{back onto the leaving node}. By hosting locally and using
Horde only for lookup, the handoff controls placement explicitly. A drain reads each worker's state,
terminates the local process, and re-creates it on a chosen survivor. The just-killed worker's registry
entry can linger briefly in the CRDT. So placement retries until the new process lands on the target,
handling the \texttt{\{:already\_started, stale\_pid\}} race.
...
policy is a pure function (its output depends only on its inputs). It is a direct transcription of
Equation~\eqref{eq:objective} with the margin, bounds, and the default-safe fallback of
Proposition~\ref{prop:safety} (Listing~\ref{lst:grace}).
...
The coordinator runs as a separate operator pod. Each reconcile cycle (one pass of the operator's
control loop) it lists the app pods, reads every pod's \texttt{/probe}, computes the grace for the
worst-case (maximum) reading, and patches the Deployment's
\texttt{terminationGracePeriodSeconds} (Listing~\ref{lst:operator}). The whole pass is wrapped so a
transient API error is logged and retried on the next tick rather than crashing the controller. This is
one of the fail-safes validated in Table~\ref{tab:faults}. It reads pods and patches the Deployment
through the pod's ServiceAccount under least-privilege RBAC (Kubernetes role-based access control, which
grants only the permissions needed).
...
\paragraph{Grace granularity: per-Deployment, not per-pod.} The prototype patches the
\emph{Deployment}'s \texttt{terminationGracePeriodSeconds}, so every replica receives the same
value: the worst case (\texttt{Enum.max}) across the pods' current readings. This is a deliberate
simplification of the per-pod ideal of Section~\ref{sec:model}. It over-provisions lightly loaded pods
to the busiest pod's need, trading a little efficiency for a single, race-free write. Two consequences
follow. First, a Deployment patch takes effect only on \emph{future} terminations, so the operator
must lead the rollout. Our 5\,s reconcile loop is comfortably ahead of the minute-scale handoffs we
measure. A burst that outran one cycle would simply reuse the previous (already conservative)
grace. That is benign staleness, never an under-grace below $g_{\min}$. Second, \emph{true} per-pod
grace is now within reach. Kubernetes 1.25+ accepts a \texttt{terminationGracePeriodSeconds} override on
the \texttt{delete} call itself. A future operator could set this per pod at termination time, removing
both the over-provisioning and the staleness. We use the Deployment patch here because it needs no
eviction-path interception and is idempotent across operator restarts (Table~\ref{tab:faults}).

\subsection{Kubernetes packaging}\label{sec:impl-pack}
The application ships as a multi-stage OCI image (a standard container image; an Elixir release on
Alpine). Its pods form a BEAM cluster through the libcluster Kubernetes strategy. This uses pod-IP
discovery against a headless Service, with each node named \texttt{grace@\$(POD\_IP)}. A
\texttt{preStop} hook turns the granted grace into useful work by calling the pod's \texttt{/drain}
endpoint, which blocks until handoff completes. Readiness and liveness use \texttt{/healthz}. A
PodDisruptionBudget plus the operator's pacing bound how many pods drain at once
(Listing~\ref{lst:deploy}). The coordinator is a second Deployment using the same image with
\texttt{GRACE\_ROLE=operator}. Full manifests are in Appendix~\ref{app:manifests}.
...
We ask ten questions. \textbf{RQ1 (correctness):} does a fixed grace lose state when the handoff
...
\textbf{RQ9 (multi-node):} does the result hold with several surviving nodes rather than a single
survivor? \textbf{RQ10 (vs.\ an adaptive baseline):} how does the model-based controller compare to a
\emph{reactive} policy that raises its grace only after observing loss?
...
single-host emulation (a threat to validity, Section~\ref{sec:discuss}). We also go beyond the two
snapshot loads of Table~\ref{tab:eval}. We sweep the handoff need over $\{10,25,40\}$\,s (below, at, and
above the 30\,s fixed grace) with three repeats each, run a three-pod rolling update, measure the
controller's overhead, analyse the grace policy's sensitivity, and scale a single node's backlog
from $1{,}000$ to $80{,}000$ stateful processes.
...
(\texttt{erts}~$17.0.2$, JIT) with Elixir~$1.20$; the application uses \texttt{Horde}~$0.10$,
\texttt{libcluster}~$3.5$, \texttt{Phoenix.PubSub}~$2.2$, \texttt{bandit}~$1.12$, \texttt{plug}~$1.19$,
and \texttt{jason}~$1.4$. The Kubernetes experiments use \texttt{kind}~$0.23$ provisioning
Kubernetes~$1.30$ (\texttt{containerd}~$1.7.15$), driven by \texttt{kubectl}~$1.30$; figures are rendered
with Python~$3.14$ and Matplotlib~$3.10$.
...
processes killed mid-handoff when the grace expired. Each cell is from 10 repeats: grace and
loss are exactly reproducible and drain time has a 95\% CI of $\le\pm0.005$\,s (max range $24$\,ms
over the ten repeats), confirming determinism (\texttt{data/results\_runs\_ci.csv}).}\label{tab:eval}
\begin{tabular}{@{}llrrr@{}}
\toprule
...
need sweep. The two fixed-grace policies lose no state while the need stays below the 30\,s grace. They
then lose $\approx$103 of the 400 processes at the heaviest sweep point (need~$40$\,s) once it exceeds
it. The adaptive controller and the 300\,s grace never lose state. The controller's granted grace tracks
the need ($16\to31\to46$\,s for need $10\to25\to40$\,s), while the fixed 300\,s reserves an order of
magnitude more than required. Variance over the three repeats is negligible: the mechanism is
deterministic under the throttle.
...
lost points, ringed). The fixed 300\,s policy is safe but sits far above the floor. It reserves
roughly $6$--$7\times$ the grace the heaviest load needs, and an order of magnitude more under light
load. The controller is the only one of the four policies in the desirable region: above the floor
(safe) yet close to it (no over-provisioning).
...
at need~$\approx$40\,s. The fixed 30\,s grace finishes fastest (90.3\,s) but loses 309 processes. The
...
\paragraph{Overhead (RQ4).} The controller is cheap. A probe reading costs $\approx 4\,\mu$s. Each
stateful process adds $\approx 6$\,KB. Handoff sustains $\approx 8{,}000$ state transfers per
second unthrottled (median of five idle-machine repeats). On Kubernetes the operator's resident
footprint is $\approx 73$\,MiB (dominated by the BEAM VM). One reconcile cycle (list pods, read every
probe, compute, and patch the Deployment) takes $\approx 74$\,ms every 5\,s, under 2\% duty. These
overheads are small relative to the per-process memory and the seconds-scale handoff they govern.
...
$2\times$, risking truncation). The estimate should therefore be biased downward. The
default-safe fallback (Section~\ref{sec:design-failsafe}) enforces this when no rate is observed.
...
$|H|=1{,}000$ to $80{,}000$ stateful processes, handing the entire backlog to one survivor
unthrottled. The two costs scale very differently. Memory grows \emph{linearly} and stays
modest, 5 to 317\,MiB across the range ($\approx$4\,KB per process), so RAM is not the binding
constraint. Effective handoff throughput (processes actually migrated per second), by contrast,
\emph{degrades super-linearly}. It falls from $\approx$3{,}400/s at $|H|=1{,}000$ to 152/s at
$20{,}000$, 57/s at $40{,}000$, and 18/s at $80{,}000$. The cause is that every migration registers and
resolves an identity in the cluster-wide delta-CRDT registry, whose per-operation cost grows with the
number of live entries. The result is a per-node ceiling under a fixed drain budget. Through
$|H|=20{,}000$ the node drains completely with zero loss (132\,s), but a 600\,s budget then runs out
mid-drain. At $40{,}000$ it migrates $34{,}368$ and abandons $5{,}632$ processes, and at $80{,}000$ the
collapse is steep, migrating only $\approx$11{,}000 and abandoning $69{,}038$. The ceiling for this
budget therefore sits near $|H|\approx30$--$35{,}000$. It is a property of the \emph{handoff
substrate}, Horde's registry, not of the controller. The controller's grace computation is
constant-time and its granted grace rises monotonically (7 to 86\,s) as throughput drops. The sweep
does, however, expose a limit of any rate-based policy. The linear model $|H|/\rho$ assumes a stable
$\rho$, yet at extreme backlogs the effective throughput collapses. So a single-snapshot rate
under-predicts the true handoff time. This reinforces RQ5's guidance to estimate $\rho$ conservatively.
...
\begin{figure}[t]
  ...
  \caption{Per-node handoff scaling ($|H|=1{,}000$ to $80{,}000$). Memory grows linearly (right), but
  effective handoff throughput collapses super-linearly as the delta-CRDT registry fills (left),
  capping the backlog a single pod can drain within a fixed budget. Through $|H|=20{,}000$ the node
  drains fully; under a 600\,s budget it then runs out mid-drain, abandoning $5{,}632$ of $40{,}000$
  and $69{,}038$ of $80{,}000$ processes.}\label{fig:scale}
\end{figure}

\paragraph{Kubernetes deployment.} We also deployed the full artifact on a Kubernetes cluster
(\texttt{kind}, a tool that runs Kubernetes inside Docker). The service ran as a three-replica
Deployment that forms a BEAM cluster through the libcluster Kubernetes strategy, the coordinator as a
separate operator Deployment, and the adaptive \texttt{preStop} hook on each pod. End-to-end, the
operator read every pod's probe and patched the Deployment's
\texttt{terminationGracePeriodSeconds} from the runtime backlog. It raised the value from 6\,s when idle
to 26\,s once roughly 190 stateful processes per pod had accumulated at a 10/s handoff rate. This
confirms the cross-layer feedback loop on a real orchestrator. The quantitative loss and efficiency
figures above come from the controlled BEAM-cluster experiment. This run validates the deployment
artifacts and the operator's control loop.
...
\texttt{kind} cluster. We injected per-packet delay with \texttt{tc netem} (a Linux tool that adds
artificial network delay) on the survivor pods' egress. We then measured, from a leaving pod, the
inter-pod round-trip time of the synchronous handoff RPC (Figure~\ref{fig:netem}; harness
\texttt{k8s/netem.sh}). The measured RTT tracks the injected delay to within $0.4$\,ms, confirming the
latency is real and on the handoff path. Each state transfer is one such round-trip, so the achievable
rate $\rho\approx 1/\mathrm{RTT}$ collapses from $\approx$21{,}700\,proc/s at sub-millisecond RTT to
$19.9$, $10.0$, and $6.7$\,proc/s at $50$, $100$, and $150$\,ms. The controller responds correctly. The
grace it would grant a representative $2{,}000$-process backlog rises from the floor ($6$\,s) to
$107$\,s at $50$\,ms RTT, then saturates the $g_{\max}=120$\,s cap beyond $100$\,ms. This validates that
$\rho$ captures genuine network cost, so the single-host efficiency numbers are conservative rather than
optimistic. It also pinpoints, for a given $g_{\max}$, the latency past which a pod's backlog must
shrink or the cap must rise: exactly hypothesis~(ii) of Proposition~\ref{prop:safety}.
...
re-read the probes, and re-patched the same grace, since assignment is idempotent. Feeding the policy an
unusable probe (a positive backlog with a zero or missing rate) returned the configured fallback
($g_{\max}=120$\,s). That is the \emph{largest} safe grace, never a small one. Revoking the operator's
API permissions mid-operation produced no crash loop (zero restarts) and left the last grace in place. A
...
\paragraph{Realistic-workload case study: Phoenix.Presence (RQ8).} We also check the mechanism on a
real, unmodified distributed feature rather than synthetic workers. We exercise
\texttt{Phoenix.Tracker}, the CRDT engine behind \texttt{Phoenix.Presence}, which is widely used to
track who-is-online across a cluster. We track $N$ presences on the leaving node. On a graceful
departure, we measure how long the survivor takes to re-converge, which is the $T_c$ term of
Equation~\eqref{eq:invariant} (Figure~\ref{fig:presence}; harness \texttt{harness/presence.exs}).
Re-convergence is $\approx$1.5\,s and, unlike handoff, essentially \emph{independent of $N$} (100 to
2{,}000 presences), because it is paced by the tracker's CRDT broadcast period, not the presence count.
Add-convergence is faster ($0.3$--$0.7$\,s). Two things follow. First, the coupling holds on a real
workload: $T_c$ is a bounded, measurable quantity the grace must cover, and the controller already
includes it. Second, the measured $\approx$1.5\,s is roughly $30\times$ our deliberately crude built-in
estimate ($50$\,ms per peer). The default margin $\sigma=5$\,s absorbs the gap. But a production
deployment should feed a \emph{measured} convergence signal into the probe rather than the heuristic.
This is an honest limitation the case study exposes.
...
\paragraph{Multi-node (RQ9).} We next check that the two-node result is not an artifact of a single
survivor. We repeat the table loads on a four-node cluster (one leaver, three survivors). The outcome
is identical. Under the heavy load the fixed 30\,s and \texttt{preStop} policies still lose exactly
\emph{40 of 160} processes, while the adaptive controller and the 300\,s grace lose none (grace 46\,s,
drain 40.2\,s). Under light load every policy is loss-free. The loss and grace numbers match the
two-node case to the second. Handoff also \emph{distributes}: draining 300 processes placed
$103/96/101$ of them on the three survivors, an even spread, because each transfer targets a random
survivor. The drain time is essentially unchanged. The bottleneck is the serial leaver, which hands its
workers off one at a time, not the survivor count. Adding survivors balances \emph{where} state lands
without changing \emph{whether} it is lost. The single-host emulation remains a threat
(Section~\ref{sec:discuss}), but the per-survivor count no longer is.

\paragraph{Versus an adaptive baseline (RQ10).} A natural objection is that \emph{any} adaptive scheme
would beat a fixed grace. Figure~\ref{fig:reactive} compares the controller against a \emph{reactive}
baseline with no runtime model. It starts at $g_{\min}$ and \emph{doubles} the grace after each
truncated termination. This is the cloud-native reflex of ``the last rollout lost state, so raise the
timeout and retry.'' On the heavy load it needs four terminations to reach a safe grace
($5\to10\to20\to40$\,s), losing $140$, $120$, and $80$ processes on the three under-provisioned
attempts (340 in total) before the fourth succeeds. The model-based controller computes the adequate
grace ($46$\,s) from the \emph{first} probe reading and loses nothing. The gap is fundamental, not
incidental. A reactive policy can only \emph{learn} the right deadline by first missing it, paying in
lost state every time the load steps up. Reading the runtime signal instead turns that trial-and-error
into a one-shot computation.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.78\linewidth]{eval_reactive.pdf}
  \caption{A reactive exponential-backoff grace (double after each truncation) must fail before it
  converges: it loses 140, 120, and 80 processes on three under-provisioned attempts
  ($g=5,10,20$\,s, 340 total) before $g=40$\,s succeeds. The model-based controller computes
  $g=46$\,s from the first reading and loses none.}\label{fig:reactive}
\end{figure}

%====================================================================
\section{Discussion and Threats to Validity}\label{sec:discuss}
\paragraph{Emulated cluster.} Our headline measurements come from a two-node cluster. We repeated
them on a \emph{four}-node cluster (one leaver, three survivors) with identical loss and grace and an
even handoff spread (RQ9), so the result is not an artifact of a single survivor. All nodes
nonetheless share one host. So the host topology and the inter-machine network are still emulated, and
a true multi-machine (or cloud) run remains future work. The Kubernetes deployment
(Section~\ref{sec:eval}) likewise ran on a single-host \texttt{kind} cluster. Two further measurements
narrow the gap. First, injecting \emph{real} inter-pod latency with \texttt{tc netem} (RQ7,
Figure~\ref{fig:netem}) shows the rate signal tracks the injected RTT. So the single-host efficiency
numbers are conservative: a real network only lowers $\rho$, which the controller compensates for with
more grace. Second, the four-node run shows the serial-leaver bottleneck, not the survivor count, sets
the drain time.
...
surfaces at extreme backlogs (RQ6). The handoff substrate's throughput collapses super-linearly, so a
single-snapshot $\rho$ over-estimates the achievable rate and the policy can under-provision grace. So
$\rho$ should be biased conservatively and per-pod backlog bounded. A fairer objection is circularity.
In the synthetic sweeps we \emph{set} $\rho$ with a throttle to isolate its effect, which risks ``the
controller works because $\rho$ is known and stable.'' The network-latency experiment (RQ7) is the
answer. There $\rho$ is not dictated but \emph{emerges} from real \texttt{tc netem} delay, the online
EWMA (not a constant) drives the grace, and the controller still tracks it, grace rising as the measured
rate falls. The mechanism therefore does not depend on $\rho$ being handed to it.

\paragraph{Stability and estimator dynamics.} The controller is \emph{feed-forward}, not a
continuously actuated control loop. The grace is computed \emph{once} per termination from a snapshot
and never acts back on the handoff rate $\rho$. So there is no feedback path that could oscillate, and
the classic closed-loop instabilities (overshoot, limit cycles) do not arise. What remains is estimator
\emph{lag}. The rate EWMA with smoothing factor $\alpha=0.3$ has residual error $(1-\alpha)^k$ after
$k$ samples following a step change in the true rate. So it tracks a new rate to within $10\%$ in about
seven handoffs and to within $3\%$ in ten. Transients shorter than that are absorbed by the margin
$\sigma$ and the bounds. At cold start, before any handoff has been observed, the probe reports the
\emph{configured} achievable rate rather than zero or an undefined value
(Section~\ref{sec:design-probe}). So even the first termination uses a sane grace, and a genuinely
missing signal triggers the default-safe fallback. The estimator thus lags toward, not past, the safe
side.
...
is future work and separate from the cross-layer coupling this paper contributes.
...
This confirms the determinism the throttle induces, so variance is not a confound here. Rolling-update
...
\begin{itemize}
  ...
  place, prefer over-provisioning to under-provisioning. Lost state is irreversible, but extra latency
  is not.
...
% Position the controller vs: dependable microservices in K8s (JISA 2024), RTilience (TSC
...
\paragraph{Adaptive deadlines: an established idea, here instantiated across layers.} Deriving a
timeout from observed behaviour rather than fixing a constant is a classic distributed-systems
pattern. Adaptive failure detectors compute a per-link timeout from heartbeat
history~\cite{chen2002qos}, and the $\varphi$ accrual detector replaces the binary timeout with a
continuous suspicion level~\cite{hayashibara2004phi}. Feedback control has long been applied to track
a setpoint in computing systems under varying load~\cite{hellerstein2004feedback}, and orchestrators
apply the same idea to autoscaling~\cite{senjab2023k8s}. We do \emph{not} claim adaptive deadlines as
novel. Our contribution is twofold. First, we observe that the orchestrator's \emph{termination}
deadline is a deadline of exactly this kind. Second, we close the specific control loop between a
Kubernetes grace period and a BEAM runtime's handoff and convergence signals. To our knowledge, no
prior system makes this coupling.

\paragraph{State migration.} Evacuating live state from a departing host is studied at every layer.
Virtual-machine and container live migration relocate an entire instance with minimal
downtime~\cite{clark2005live,nadgowda2017voyager}. Stateful stream processors checkpoint and migrate
operator state during reconfiguration and rescaling~\cite{carbone2015snapshots,carbone2015flink,
hoffmann2019megaphone}. Our handoff is the same class of operation one level up, namely per-process
state moved between runtime nodes. But the contribution is not the migration mechanism (Horde provides
it). It is ensuring the orchestrator's kill deadline is long enough for that migration to finish, which
the migration literature assumes but Kubernetes does not guarantee.
...
% convergence signals -- they use configured/static timeouts. That is the gap we target.
...
timeouts~\cite{akkacoordinated}. Orleans, a virtual-actor runtime~\cite{bernstein2014orleans},
performs graceful silo shutdown with grain deactivation~\cite{orleans}. In both, however, the
timeouts are \emph{configured constants}, internal to the runtime, and are not fed back to the
orchestrator's termination grace period. So the same grace gap we target can occur. Our controller
differs by closing an explicit cross-layer feedback loop. The grace period is \emph{derived} at
...
\paragraph{What porting to another runtime requires.} Generality here is a design property, not just
an aspiration, because the cross-layer machinery is cleanly separated from the runtime. The grace
policy (Equation~\eqref{eq:objective}), the safety invariant, the operator's reconcile-and-patch loop,
and the rollout pacing are entirely runtime-agnostic. They consume one small interface, a
\emph{drain-progress signal} $\langle$backlog $|H|$, achievable rate $\rho$, convergence
estimate $T_c\rangle$, and emit a per-pod \texttt{terminationGracePeriodSeconds}. Only two pieces are
runtime-specific: a read-only \emph{probe} that reports that triple, and a \emph{termination hook}
that drains and blocks until handoff completes (in our prototype, \texttt{Probe}, $\approx$80 lines,
and \texttt{Shutdown}+\texttt{Handoff}). Everything else is reused unchanged. The signal exists in the
other runtimes too. Akka's cluster sharding exposes the entities being rebalanced and
\emph{CoordinatedShutdown} its phase progress, and Orleans' silo shutdown exposes outstanding grain
deactivations. Each is exactly the backlog/progress a probe would report. Porting the controller is
therefore implementing that probe and hook against the target runtime's APIs and reusing the operator
and policy verbatim. We benchmark only the BEAM realisation and leave the others as this concrete (but
unimplemented) recipe rather than a vague claim of portability.
...
A fixed termination grace period is a poor fit for stateful handoff. The orchestrator
...
convergence signals and paces the rolling update accordingly. It is realised as an open artifact on
...
generalises the principle to other actor runtimes, and pursues the broader cross-layer agenda:
coordinating disruption budgets with cluster quorum, compiling a single deployment specification to
both layers, and aligning restart budgets with backoff.
...
elapses. The latter case is exactly the truncated handoff a too-short grace produces.
Listing~\ref{lst:rbac} gives the Kubernetes objects omitted from Section~\ref{sec:impl-pack}: the
headless Service for BEAM discovery, the PodDisruptionBudget, and the operator's least-privilege RBAC.
