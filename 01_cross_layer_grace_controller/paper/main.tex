...
application runtime's. When such a service is upgraded, or a machine is withdrawn for maintenance, an
...
grace period is structurally mismatched to this task: set it too low and forced termination interrupts
the migration, losing state and availability; set it defensively high and every instance slows the
rollout. The time actually required is not a constant---it grows with the amount of state to migrate and
the load on the cluster---so no single value is right for all instances or all moments. We present a
...
shutdown window to the runtime's migration and convergence time, provide an open reference
implementation on Kubernetes for Erlang/BEAM clusters, and evaluate it against fixed and
over-provisioned baselines under fault injection. Where a fixed short grace loses about 25\% of the
...
establishes a feedback path across the orchestrator/runtime boundary
(Table~\ref{tab:existing})---which, as Section~\ref{sec:intro} set out, is where the information that
should size the deadline and the control that enforces it are split. Building that path is exactly
...
measure, and a burst that outran one cycle would simply reuse the previous (already conservative)
grace---benign staleness, never an under-grace below $g_{\min}$. Second, \emph{true} per-pod grace is
now within reach: Kubernetes 1.25+ accepts a \texttt{terminationGracePeriodSeconds} override on the
\texttt{delete} call itself, which a future operator could set per pod at termination time, removing
...
it; the adaptive controller and the 300\,s grace never lose state. The controller's granted grace tracks the need ($16\to31\to46$\,s for need
...
load. The controller is the only one of the four policies in the desirable region: above the floor (safe) yet close to
...
stateful process adds $\approx 6$\,KB; and handoff sustains $\approx 8{,}000$ state transfers per
second unthrottled (median of five idle-machine repeats). On Kubernetes the operator's resident
footprint is $\approx 73$\,MiB (dominated
by the BEAM VM), and one reconcile cycle---list pods, read every probe, compute, and patch the
Deployment---takes $\approx 74$\,ms every 5\,s, under 2\% duty. These overheads are small relative to
the per-process memory and the seconds-scale handoff they govern.
...
$20{,}000$, 57/s at $40{,}000$, and 18/s at $80{,}000$, because every migration registers and resolves
an identity in the cluster-wide delta-CRDT registry, whose per-operation cost grows with the number of
live entries. The result is a per-node ceiling under a fixed drain budget: through $|H|=20{,}000$ the
node drains completely with zero loss (132\,s), but a 600\,s budget then runs out mid-drain---at
$40{,}000$ it migrates $34{,}368$ and abandons $5{,}632$ processes, and at $80{,}000$ the collapse is
steep, migrating only $\approx$11{,}000 and abandoning $69{,}038$. The ceiling for this budget
therefore sits near $|H|\approx30$--$35{,}000$. It is a property of the \emph{handoff
substrate}---Horde's registry---not of the controller, whose grace computation is constant-time and
whose granted grace rises monotonically (7 to 86\,s) as throughput drops. It does, however, expose a
limit of any rate-based policy: the linear model $|H|/\rho$ assumes a stable $\rho$, yet at extreme
backlogs the effective throughput collapses, so a single-snapshot rate under-predicts the true handoff
time---reinforcing RQ5's guidance to estimate $\rho$ conservatively. The operational remedy is the
same either way: bound the per-pod stateful backlog and scale \emph{horizontally} (more replicas,
smaller $|H|$ each), which the Kubernetes deployment supports directly; raising the per-node ceiling by
sharding the registry is future work. Raw data is in \texttt{data/results\_scale.csv} (harness
\texttt{harness/scale.exs}).
...
delay to within $0.4$\,ms, confirming the latency is real and on the handoff path. Because each state
transfer is one such round-trip, the achievable rate $\rho\approx 1/\mathrm{RTT}$ collapses from
$\approx$21{,}700\,proc/s at sub-millisecond RTT to $19.9$, $10.0$, and $6.7$\,proc/s at $50$, $100$,
...
\paragraph{Multi-node (RQ9).} To check that the two-node result is not an artifact of a single
...
drain 40.2\,s), and under light load every policy is loss-free---the loss and grace numbers match the
...
survivor. The drain time is essentially unchanged because the bottleneck is the serial leaver---one node hands its
...
truncated termination---the cloud-native reflex of ``the last rollout lost state, so raise the timeout
and retry.'' On the heavy load it needs four terminations to reach a safe grace
...
lost state every time the load steps up, whereas reading the runtime signal turns that trial-and-error
...
\paragraph{Emulated cluster.} Our headline measurements come from a two-node cluster, and we repeated
...
narrow the gap: injecting \emph{real} inter-pod latency with \texttt{tc netem} (RQ7,
Figure~\ref{fig:netem}) shows the rate signal tracks the injected RTT---so the single-host efficiency
numbers are conservative, since a real network only lowers $\rho$, which the controller compensates
for with more grace---and the four-node run shows the serial-leaver bottleneck, not the survivor
count, sets the drain time.
...
under-provision grace---so $\rho$ should be biased conservatively and per-pod backlog bounded. A
fairer objection is circularity: in the synthetic sweeps we \emph{set} $\rho$ with a throttle to
isolate its effect, which risks ``the controller works because $\rho$ is known and stable.'' The
network-latency experiment (RQ7) is the answer: there $\rho$ is not dictated but \emph{emerges} from
real \texttt{tc netem} delay, the online EWMA (not a constant) drives the grace, and the controller
still tracks it---grace rising as the measured rate falls. The mechanism therefore does not depend on
$\rho$ being handed to it.
...
and never acts back on the handoff rate $\rho$, so there is no feedback path that could oscillate---the
classic closed-loop instabilities (overshoot, limit cycles) do not arise. What remains is estimator
...
novel. Our contribution is the observation that the orchestrator's \emph{termination} deadline is a
deadline of exactly this kind, and the closing of the specific control loop between a Kubernetes grace
period and a BEAM runtime's handoff and convergence signals---a coupling that, to our knowledge, no
prior system makes.
...
hoffmann2019megaphone}. Our handoff is the same class of operation one level up---per-process state
moved between runtime nodes---but the contribution is not the migration mechanism (Horde provides it):
it is ensuring the orchestrator's kill deadline is long enough for that migration to finish, which the
migration literature assumes but Kubernetes does not guarantee.
...
and the rollout pacing are entirely runtime-agnostic: they consume one small
interface---a \emph{drain-progress signal} $\langle$backlog $|H|$, achievable rate $\rho$, convergence
