...
\label{subsec:m1-ilustrasi}
\begin{enumerate}[nosep,leftmargin=2em]
  \item Saat kubelet menanyakan kesehatan lewat \texttt{/healthz}, aplikasi tidak menjawab
        asal ``hidup''.
  \item Ia memeriksa \emph{supervision tree} miliknya (peta \emph{process} dan pengawasnya):
        adakah bagian penting yang macet atau terus-menerus \emph{crash}?
  \item Bila ada, ia menjawab ``tidak sehat'', sehingga Kubernetes berhenti mengirim lalu
        lintas ke \emph{pod} itu atau me-\emph{restart}-nya.
\end{enumerate}
\noindent Intinya: jawaban \emph{probe} berasal dari kesehatan internal yang sebenarnya,
bukan sekadar \emph{port} yang terbuka---kebalikan dari Masalah~1.
...
\subsection{Ilustrasi Sederhana}
\label{subsec:m2-ilustrasi}
\begin{enumerate}[nosep,leftmargin=2em]
  \item Ketika sebuah kegagalan muncul, sebuah ``penilai'' mengelompokkannya: kegagalan
        kecil di tingkat \emph{process}, atau besar di tingkat mesin/sumber daya.
  \item Kalau kecil, OTP (supervisor) yang menanganinya dan Kubernetes tidak diberi tahu.
        Kalau besar, OTP mundur dan Kubernetes yang me-\emph{restart} \emph{pod}.
\end{enumerate}
\noindent Intinya: satu kegagalan ditangani tepat oleh satu lapis, sehingga tidak ada kerja
pemulihan ganda.
...
\subsection{Ilustrasi Sederhana}
\label{subsec:m3-ilustrasi}
\begin{enumerate}[nosep,leftmargin=2em]
  \item Sebelum mematikan \emph{pod}, pengendali mengukur dua hal: berapa banyak
        \emph{process} yang harus dipindah, dan seberapa cepat pemindahannya.
  \item Dari situ ia menghitung tenggat (\emph{grace}) yang cukup---misalnya bila perlu
        45~detik, beri 50~detik---bukan angka tetap 30~detik.
\end{enumerate}
\noindent Intinya: tenggat disesuaikan dengan beban nyata, sehingga \emph{handoff} selalu
selesai dan tak ada \emph{state} yang hilang (kebalikan dari Masalah~4).
...
\subsection{Ilustrasi Sederhana}
\label{subsec:m4-ilustrasi}
\begin{enumerate}[nosep,leftmargin=2em]
  \item Sebuah \emph{operator} (program kecil yang mengawasi dan mengatur sumber daya
        Kubernetes) terus memantau keanggotaan \emph{cluster} BEAM.
  \item Ia menandai \emph{pod} ``siap'' hanya setelah \emph{pod} benar-benar bergabung ke
        \emph{cluster}; dan saat \emph{pod} hendak dimatikan, ia menahan penghapusan sampai
        \emph{handoff} selesai.
\end{enumerate}
\noindent Intinya: Kubernetes baru mengirim lalu lintas ketika \emph{pod} benar-benar siap,
dan tidak mematikannya sebelum datanya aman---menutup Masalah~3 dan~4.
...
\subsection{Ilustrasi Sederhana}
\label{subsec:m5-ilustrasi}
\begin{enumerate}[nosep,leftmargin=2em]
  \item Saat jaringan terpecah, tiap sisi berebut mengambil satu ``kunci'' tunggal yang
        disimpan Kubernetes (\emph{Lease}, didukung etcd---penyimpanan pusat Kubernetes).
  \item Hanya satu sisi yang berhasil memegang kunci itu, dan hanya sisi itu yang berhak
        melanjutkan; sisi lain berhenti (\emph{fence}) sehingga tidak ada salinan ganda.
\end{enumerate}
\noindent Intinya: ada satu wasit tepercaya yang memutuskan sisi mana yang berkuasa,
mencegah \emph{split-brain} (kebalikan dari Masalah~5).
...
\subsection{Ilustrasi Sederhana}
\label{subsec:m6-ilustrasi}
\begin{enumerate}[nosep,leftmargin=2em]
  \item Pengembang menulis \emph{satu} berkas yang menyatakan niat pemulihan (misalnya
        ``\emph{process} ini menyimpan \emph{state}, butuh kuorum sekian, pindahkan---jangan
        sekadar di-\emph{restart}'').
  \item Sebuah \emph{compiler} menerjemahkan berkas itu menjadi dua keluaran sekaligus:
        konfigurasi OTP dan berkas Kubernetes (\emph{manifest}).
\end{enumerate}
\noindent Intinya: satu sumber kebenaran membuat kedua lapis selalu sinkron---ubah sekali,
keduanya ikut berubah, sehingga tidak ada \emph{drift}.
...
\subsection{Ilustrasi Sederhana}
\label{subsec:m7-ilustrasi}
\begin{enumerate}[nosep,leftmargin=2em]
  \item Pengendali menghitung berapa \emph{pod} minimum yang harus tetap hidup agar
        \emph{cluster} BEAM tidak kehilangan kuorum (jumlah suara mayoritas yang diperlukan
        agar \emph{cluster} tetap berwenang).
  \item Angka itu ditulis ke \emph{PodDisruptionBudget} (aturan Kubernetes tentang berapa
        \emph{pod} boleh dimatikan saat pemeliharaan).
  \item Ketika ada \emph{drain} atau pembaruan, Kubernetes menolak mematikan \emph{pod} bila
        akan melanggar batas itu.
\end{enumerate}
\noindent Intinya: pemeliharaan terjadwal tak pernah memutus kuorum BEAM.
...
\subsection{Ilustrasi Sederhana}
\label{subsec:m8-ilustrasi}
\begin{enumerate}[nosep,leftmargin=2em]
  \item Pengendali mengamati seberapa sering kegagalan terjadi, lalu menyetel jatah
        \emph{restart} OTP secara adaptif (mengetat saat sering gagal).
  \item Selama masih dalam jatah, OTP yang menangani; begitu jatah habis (kegagalan
        terus-menerus), VM keluar sehingga Kubernetes me-\emph{restart} \emph{pod} dengan
        jeda yang makin lama (\emph{backoff}).
\end{enumerate}
\noindent Intinya: batas ``kapan menyerah ke Kubernetes'' ditentukan secara terukur dan
mengikuti beban, bukan angka tetap (kebalikan dari Masalah~2).
...
\subsection{Ilustrasi Sederhana}
\label{subsec:m9-ilustrasi}
\begin{enumerate}[nosep,leftmargin=2em]
  \item Alih-alih \emph{probe} TCP, sebuah pengumpul (\emph{aggregator}) membaca beberapa
        tanda vital VM BEAM: seberapa sibuk penjadwal, panjang antrean kerja
        (\emph{run-queue}), tumpukan pesan di \emph{mailbox}, dan seberapa sering
        \emph{process} \emph{crash}.
  \item Dari semua itu ia menghitung satu skor ``apakah VM masih membuat kemajuan?''; bila
        skor rendah, \emph{pod} dinyatakan tidak sehat.
\end{enumerate}
\noindent Intinya: mekanisme ini menangkap keadaan ``hidup tetapi macet'' yang lolos dari
\emph{probe} biasa (kebalikan dari Masalah~1).
